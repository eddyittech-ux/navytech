/* ui.js v0.4.8 — NavyTech
   ...
   - Tema sol/luna
   - Resumen avanzado
   - Contactos / Acuerdos / Prácticas / Juegos / Luces
   - Fix duplicado addLightBtn → btnAddLight
   - Mapa de caritas COMPLETO
*/
(() => {
  // Utils
  ...
  // ===== Toast =====
  function toast(msg, type='info'){ const host=qs('#toastHost')||document.body; const el=document.createElement('div'); el.className=`gs-card px-4 py-2 text-sm border-l-4 ${type==='error'?'border-red-400':type==='success'?'border-emerald-400':'border-[#C7A740]'}`; el.textContent=msg; host.appendChild(el); setTimeout(()=>el.remove(),3200); }
  ...
  function attachLoginHandlers(attempt=0){
    const form=qs('#loginForm'); const btn=qs('#loginBtn');
    if(!form || !btn){ if(attempt<20) return setTimeout(()=>attachLoginHandlers(attempt+1),100); return; }
    if(form.__bound) return; form.__bound=true;
    form.addEventListener('submit', async(e)=>{ e.preventDefault(); await doLogin(); });
  }
  async function doLogin(){
    const email=qs('#emailInput')?.value?.trim(); const password=qs('#passwordInput')?.value||''; const btn=qs('#loginBtn');
    if(!email || !password) return toast('Completa email y password','error');
    try{ btn.disabled=true; btn.textContent='Entrando…'; const user=await window.NT.auth.signIn(email,password); toast('Sesión iniciada','success'); await refreshAuthUI(user); }
    catch(err){ console.error(err); toast(err.message||'No se pudo iniciar sesión','error'); }
    finally{ btn.disabled=false; btn.textContent='Entrar'; }
  }
  onReady(async()=>{ attachLoginHandlers(); window.NT?.auth?.onAuth?.((u)=>refreshAuthUI(u)); const u=await window.NT?.auth?.getUser()?.catch(()=>null); refreshAuthUI(u); });
  // fallback del botón
  ...
  const emoScore = {
    ...
    meh: 0, cansado: -1, nervioso: -1,
    triste: -2, muy_triste: -3, frustrado: -2, estresado: -2, ansioso: -2, furioso: -3
  };
  ...
  function gaugeSVG(value=7, size=90){
    const pct=Math.max(1,Math.min(10,Number(value)))/10; const r=size/2-10, cx=size/2, cy=size/2, C=2*Math.PI*r, dash=(C*pct).toFixed(1);
  ...
  function sparkline(values=[], color='#999', w=300, h=60){
    ...
    const pts = values.map((v,i)=>{
      const x=pad + i*step; const t=(v-min)/(max-min || 1); const y=h - pad - t*(h-2*pad);
  ...
  async function renderResumen(){
    ...
    try{
      ...
      const percent = 0; // placeholder hasta metas
      ...
      const byDayPerson = (person) => {
        ...
        lights.filter(l=>l.who===person).forEach(l=>{
          const d=l.light_on, s=emoScore[l.emotion] ?? 0, v=map.get(d) || {sum:0,n:0};
          v.sum+=s; v.n++; map.set(d,v);
        });
  ...
  // ===== Arranque =====
  onReady(()=>{ initTheme(); if(!location.hash) location.hash='#/resumen'; parseRoute(); });
})();
