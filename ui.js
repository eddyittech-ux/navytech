...
(() => {
  ...
  async function refreshAuthUI(user){
    const logged=!!user;
    $$(authCard(),!logged); $$(appViews(),logged); $$(mainNav(),logged); $$(authActions(),logged);
  ...
  /* ====== A PARTIR DE AQUÍ: renderizadores (idénticos a los que ya te pasé) ======
     Dejé el mismo código de Resumen, Contactos, Acuerdos, Prácticas, Juegos y Luces.
     No los repito para no hacer esta respuesta kilométrica, pero no hay cambios
     respecto al turno anterior, solo la parte del router y getViews(). 
     Si igual quieres que los pegue otra vez completos aquí, me dices y te los
     escribo de nuevo 1:1. 
  */
  ...
  async function renderLightsInit(){ const today=new Date(); if(lightsRangeType) lightsRangeType.value='week'; if(lightsRangeStart) lightsRangeStart.value=rangeFrom('week', today); await renderLights(); }
  ...
  async function renderLights(){ if(!lightsList) return; lightsList.innerHTML=`<div class="text-sm opacity-70">Cargando...</div>`; try{ const type=lightsRangeType?.value||'week'; const start=lightsRangeStart?.value||rangeFrom(type, new Date()); const dStart=new Date(start); const dEnd=new Date(start); if(type==='week'){ dEnd.setDate(dStart.getDate()+6);} else { dEnd.setMonth(dStart.getMonth()+1); dEnd.setDate(dEnd.getDate()-1); } const from=start, to=dEnd.toISOString().slice(0,10); const items = await window.NT.lights.listLights({ from, to }); const counts = {Rojo:0,'Ámbar':0,Verde:0,Azul:0}; items.forEach(i=> counts[i.color]=(counts[i.color]||0)+1); if(lightsStats) lightsStats.innerHTML = Object.entries(counts).map(([k,v]) => `<div class="gs-card p-3 flex items-center justify-between"><div class="flex items-center gap-2">${colorDot(k)} <span>${k}</span></div><div class="text-2xl font-semibold">${v}</div></div>`).join(''); if(!items.length){ lightsList.innerHTML=`<div class="text-sm opacity-70">Sin entradas.</div>`; return; } lightsList.innerHTML = items.map(l=>`<div class="gs-card p-4 flex items-start justify-between"><div><div class="font-medium flex items-center gap-2">${colorDot(l.color)} <span>${esc(l.action)}</span></div><div class="text-xs opacity-70">${esc(l.who)} · ${esc(l.emotion)} · ${esc(l.light_on)}</div>${l.notes?`<div class="mt-2 text-xs opacity-80">${esc(l.notes)}</div>`:''}</div><div class="flex items-center gap-2"><button class="gs-btn text-xs" data-edit-light="${l.id}">Editar</button><button class="gs-btn text-xs" data-del-light="${l.id}">Borrar</button></div></div>`).join(''); qsa('[data-edit-light]').forEach(b=> b.addEventListener('click',()=>{ const id=b.getAttribute('data-edit-light'); const it=items.find(x=>x.id===id); openLightModal(it); })); qsa('[data-del-light]').forEach(b=> b.addEventListener('click',async()=>{ const id=b.getAttribute('data-del-light'); if(!confirm('¿Eliminar entrada?')) return; try{ await window.NT.lights.deleteLight(id); toast('Eliminada','success'); renderLights(); }catch(e){ console.error(e); toast('Error al eliminar','error'); }})); }catch(e){ console.error(e); lightsList.innerHTML=`<div class="text-sm text-red-300">Error al cargar</div>`; } }
  function openLightModal(item=null){ if(!lightModal) return; lightModalTitle.textContent=item?'Editar entrada':'Nueva entrada'; $$(deleteLightBtn,!!item); lightId.value=item?.id||''; lightDate.value=item?.light_on||new Date().toISOString().slice(0,10); lightColor.value=item?.color||'Verde'; lightWho.value=item?.who||'Eddy'; lightEmotion.value=item?.emotion||'meh'; lightAction.value=item?.action||''; lightNotes.value=item?.notes||''; lightModal.showModal(); }
